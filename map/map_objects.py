...
class MapObject:
    ...
    y_offset: int = 0
    x_offset: int = 0
    base_url: str = "https://raw.githubusercontent.com/whitewillem/PogoAssets/main/uicons-outline/{}.png"
...
class Pokemon(MapObject):
    def __init__(self, data: tuple, dmap: Map):
        ...
        if costume:
            uicon += f"_c{costume}"
        self.url = self.base_url.format(f"pokemon/{uicon}")
...
class Gym(MapObject):
    def __init__(self, data: tuple, dmap: Map):
        ...
        self.url = self.base_url.format(f"gym/{uicon}")
...
class Pokestop(MapObject):
    def __init__(self, data: tuple, dmap: Map):
        ...
        self.url = self.base_url.format("pokestop/0")
...
class Grunt(MapObject):
    def __init__(self, data: tuple, dmap: Map):
        ...
        self.stop.url = self.base_url.format("pokestop/0_i")
        self.url = self.base_url.format(f"invasion/{grunt_id}")
...
class _RaidEgg(MapObject):
    def __init__(self, data: tuple, dmap: Map):
        ...
        self.url = self.base_url.format(f"raid/egg/{level}")
        self.size = dmap.get_marker_size()
